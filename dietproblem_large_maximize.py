...
cost = dict([(x[0], float(x[nutrient_names.index('Protein')])) for x in large_diet_list])

lp_prob = LpProblem('Diet Problem', LpMaximize)
...
print("Total Protein in diet = $%.2f" % value(lp_prob.objective))